...
chunk_folder_path = create_data_out_path("audio_analysis_chunks")


def get_audio_analysis(chunk):
    ...
    if not os.path.exists(out_path):
        ...
        chunk_data = chunk.uri.parallel_apply(
            lambda uri: pd.Series(sp.audio_analysis(uri)[0])
        )
...
analysis_chunks = [get_audio_analysis(chunk) for chunk in chunks]
print("done fetching data")

# %%
audio_analysis = pd.concat(analysis_chunks)
# %%
audio_analysis.to_csv(create_data_out_path("audio_analysis.csv", index=False))
